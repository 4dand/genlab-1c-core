...
**Open-source бенчмарк LLM для генерации кода 1С:Предприятие (BSL)**

Оценка Claude, GPT, Gemini · Методика SMOP · Контекст метаданных через MCP · Docker
...
Open-source вычислительное ядро платформы **GenLab-1C** — бенчмарк для оценки больших языковых моделей (LLM) при генерации кода на встроенном языке 1С:Предприятие 8 (BSL). Разработан в рамках НДР-2026:
...
Поддерживает модели **Claude Opus 4.5**, **GPT-5.2 Codex**, **Gemini 3 Flash** через единый API-шлюз OpenRouter. Реализует полный пайплайн: пакетная генерация кода → экспертная оценка по методике SMOP → статистический анализ → визуализация и отчёты (HTML / LaTeX / JSON).
